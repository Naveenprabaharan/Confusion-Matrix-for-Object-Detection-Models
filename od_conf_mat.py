    ...
    if gt:
        ...
        for i in range(len(clas)):
            try:
                ...
                print(f"Warning: Class '{clas[i]}' not found in class_names list. Ignoring this class.")
        if temp:
            temp = np.array(temp)
            x[:, 0] = temp
            x = x.astype(np.int32)
        else:
            print("Error: No valid classes found in the ground truth.")
    else:
        clas = x[:, -1]
        temp = []
        for i in range(len(clas)):
        ...
        if temp:
            ...
            print("Error: No valid classes found in the predictions.")
...
class_names = ['Goggle','Helmet','Person','Shoe','Vest']

gt = glob.glob(os.path.join('sample_data/gt/', '*.txt'))
print('gt: ', len(gt))
pred = glob.glob(os.path.join('/home/cognitica_ai_user/NPS/Github/Confusion_Matrix_for_Objecti_Detection_Models/sample_data/pred/', '*.txt'))
...
for i in range(len(gt)):
    ...
    for j in range(len(pred)):
        print('pred :', pred[j])
        if ( (gt[i].split('/'))[-1] == (pred[j].split('/'))[-1] ):
            print('same')
            y_p = np.asarray(read_txt(pred[j], pred=True))  
